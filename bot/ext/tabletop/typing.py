...
class Typing(commands.Cog):
    ...
    tips = [
        "Always start with the correct finger position on based on your keyboard configuration."
    ]
    ...
    async def init(self):
        with open("bot/assets/lord-of-the-rings.txt", "r", encoding="utf-8") as f:
            self.lines = [
                ln for ln in f.readlines() if all([c in printable for c in ln])
            ]
    ...
    @typing.command(name="lookalike")
    async def lookalike(self, ctx):
        await ctx.reply(
            embed=Embed().set_image(url="attachment://text.png"),
            file=File(self.get_text_image(self.extract()), "text.png"),
        )
    ...
    def get_text_image(self, text) -> BytesIO:
        ...
        with Image.new("RGBA", (w, h * len(text)), (47, 49, 54, 255)) as img:
    ...
    async def start_race(self, commander, ctx, match: shadchan.MatchInstance):
        ...
        embed.set_author(name="TYPING RACE ➖ STARTING")
        m = await ctx.channel.send(embed=embed)

        embed.description = "God Speed."
        embed.set_image(url="attachment://text.png")
        embed.set_thumbnail(
            url="https://raw.githubusercontent.com/twitter/twemoji/master/assets/72x72/2328.png"
        )
        embed.add_field(
            name="Progress", value="You: <a:loadin_color:902978712503472211>"
        )
        if match.options.mode != shadchan.MatchModes.singleplayer:
            embed.add_field(
                name="\u200b", value=f"Opponent: <a:loadin_color:902978712503472211>"
            )
        await asyncio_sleep(3)
        await m.delete()
        await match.wait_other()
        m = await ctx.channel.send(
            file=File(self.get_text_image(text), filename="text.png"), embed=embed
        )

        @match.on_emit("player_done")
        async def update_finished(player, symbol):
            if commander.id == player.id:
                ...
                embed.set_field_at(
                    ...
                    value=f"Opponent: {symbol}",
                )
            await m.edit(embed=embed)

        # resolve the delay in sending embeds and subtract accordingly
        st = time()
        ...
        try:
            msg = await self.bot.wait_for(
                "message",
                check=lambda m: m.author.id == commander.id
                and m.channel.id == ctx.channel.id,
                timeout=1.5*len(text),
            )
        except TimeoutError:
            match.emit("player_done", commander, "<:grey_check:847797996137742396>")
            msg = type("\u200b", (), {"content": ""})
        else:
            match.emit("player_done", commander, "<a:done:903006503538143323>")
        ...
        async def post_performance(player, wpm, taken, percent_acc, score):
            if player.id == commander.id:
                display = "Your"
            else:
                display = await Hearsay.resolve_name(player)
            idx = 0 if display == "Your" else 1
            scoreboard.set_field_at(
                idx,
                name=scoreboard.fields[idx].name,
                value=(
                    f"**[{display} Performance](https://google.com)**\n```diff\n+ 🕐 {round(taken)} seconds"
        ...
        await match.emit_wait_for(
            "post_performance", commander, wpm, taken, percent_acc, score
        )
        scoreboard.set_author(name=f"TYPING RACE ➖ {'VICTORY' if res else 'DEFEAT'}!")
        scoreboard.set_thumbnail(url=self.get_badge(res, score))
        scoreboard.set_footer(
            text=f"You scored {5*wpm}pts for speed and {round(45*acc)} pts for accuracy."
        )
    ...
    async def typing_race(self, ctx, mode: int = shadchan.MatchModes.unknown):
        """
        ...
        await p.lineup(
            ...
            ctx,
            self.bot.loop,
            lambda *args: self.bot.loop.create_task(self.start_race(*args)),
            shadchan.MatchOptions(mode),
        )

...
class SplitOrSteal(commands.Cog):
    ...
    async def start_splitorsteal(self, commander: Member, ctx: Context, match: shadchan.MatchInstance):
        ...
        async def register_player(player):
            players.append(await Hearsay.resolve_name(player, "**[%name](https://google.com/)** %badges\n"))
        await match.emit_wait_for("register_player", commander)

        embed.add_field(name="Game Details",
                        value=''.join(players))
        ...
        while rep <= 5:
            ...
            view = YesNoUi(ctx)
            await ctx.send(embed=Embed(description="*Would you like to continue this conversion for 60 more seconds?*", color=0x2F3136), view=view)
            await view.wait()
            await match.wait_other()
            cond = await match.conclude(view.choice, lambda a, b: a and b)
            if cond:
                await ctx.send(embed=Embed(description="*Both players chose to continue the conversation.*", color=0x2F3136))
                rep += 1
            else:
                await ctx.send(embed=Embed(description="*One or both of the players chose to stop the conversation here.*", color=0x2F3136))
                break
        view = SOSUi(ctx)
        embed = Embed(color=0x2F3136)
        await ctx.send(embed=Embed(description="*What do you do?*", color=0x2F3136), view=view)
        await view.wait()
    ...
    async def splitorsteal(self, ctx):
        ...
        opts = shadchan.MatchOptions(shadchan.MatchModes.unknown,
                disabled_modes=[shadchan.MatchModes.singleplayer, shadchan.MatchModes.lan])
        await p.lineup(ctx.author, ctx, self.bot.loop,

...
class PictionaryTutorial(menus.Menu):
    async def send_initial_message(self, ctx, _):
        prefix = ctx.prefix
        self.my_pages = (('Step 1:', f'Initiate a lobby using the command \n`{prefix}start <mode> <rounds> <participants>`\nAll participants have to react to the message with the emoji given to start the game!',
                          "https://i.gyazo.com/f641c27eac788ebb888455ba58826a33.png"),

                         ('Step 2:', 'If its your turn to draw, the bot will DM you the details, just draw and submit by replying to the dm!',
                          "https://i.gyazo.com/e4be08116bdd0fedab95c70f200f5dee.png"),

                         ('Step 3:', 'Is it not your turn to draw?!, son, its still your turn to guess, wake yourself up!',
                          "https://i.gyazo.com/bd8c8a8b42e6da796317a87077bced4a.png"),
                         ("You've reached the end! wasn't so hard was it.", f'[Join our support server for further support!](https://discord.gg/UmnzdPgn6g)', "https://scrip.pharmaintelligence.informa.com/-/media/editorial/scrip/2019/07/sc1907_the-end_1422417179_1200.jpg"))
        self.icons = ('1f4d8', '1f4d5', '1f4d7', '1f4d9')
        self.index = 0
        return await ctx.reply(embed=discord.Embed(description=self.my_pages[self.index][1], color=discord.Colour.random()).set_image(url=self.my_pages[self.index][2]).set_author(name=self.my_pages[self.index][0]).set_thumbnail(url=f"https://raw.githubusercontent.com/twitter/twemoji/master/assets/72x72/{random.choice(self.icons)}.png"))
    ...
    async def on_right_arrow(self, payload):
        if not self.index >= len(self.my_pages):
            self.index += 1
            await self.message.edit(embed=discord.Embed(description=self.my_pages[self.index][1], color=discord.Colour.random()).set_image(url=self.my_pages[self.index][2]).set_author(name=self.my_pages[self.index][0]).set_thumbnail(url=f"https://raw.githubusercontent.com/twitter/twemoji/master/assets/72x72/{random.choice(self.icons)}.png"))
...
class Pictionary(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        with open('bot/ext/pictionary/src/themes.json', 'r') as f:
    ...
    @commands.group(name="pictionary", aliases=("pic",), invoke_without_command=True)
...
def setup(bot):
    ...
    print('[PRIORITY] Pictionary Game is now loaded')

...
# Bot constructor
bot = commands.Bot(command_prefix=get_prefix, intents=intents, status=Status.offline)
...
async def is_beta(ctx: Context):
    cond = (
        ...
        and ctx.guild.id in PRIVILEGED_GUILDS
        or not ctx.command.cog_name.lower() in BETA_MODULES
    )
    if not cond:
        await ctx.reply(
            embed=Embed(
                title="Slow Down...!",
                description="This module is in beta-testing! Please join any of the privileged servers to test the module.",
                color=Colour.EXCEPTION,
            )
        )
...
async def update_status():
    ...
    await bot.change_presence(
        ...
        activity=Activity(
            type=ActivityType.listening, name=f"~help | {len(bot.guilds)} guilds"
        ),
    )
